...
func TestDisasmInstructions(t *testing.T) {
	examples := []struct {
		...
		{[]uint16{instruction(0x0, 0x01, 0x02), 0x0000, 0x0000}, 1, "JSR C"},

		// Sanity check with examples from specification
		{[]uint16{0x7c01, 0x0030, 0x0000}, 2, "SET A, 0x0030"},
		{[]uint16{0x7de1, 0x1000, 0x0020}, 3, "SET [0x1000], 0x0020"},
		{[]uint16{0x7803, 0x1000, 0x0000}, 2, "SUB A, [0x1000]"},
		{[]uint16{0xc00d, 0x0000, 0x0000}, 1, "IFN A, 0x10"},
		{[]uint16{0x7dc1, 0x001a, 0x0000}, 2, "SET PC, 0x001a"},
		{[]uint16{0xa861, 0x0000, 0x0000}, 1, "SET I, 0x0a"},
		{[]uint16{0x7c01, 0x2000, 0x0000}, 2, "SET A, 0x2000"},
		{[]uint16{0x2161, 0x2000, 0x0000}, 2, "SET [0x2000+I], [A]"},
		{[]uint16{0x8463, 0x0000, 0x0000}, 1, "SUB I, 0x01"},
		{[]uint16{0x806d, 0x0000, 0x0000}, 1, "IFN I, 0x00"},
		{[]uint16{0x7dc1, 0x000d, 0x0000}, 2, "SET PC, 0x000d"},
		{[]uint16{0x9031, 0x0000, 0x0000}, 1, "SET X, 0x04"},
		{[]uint16{0x7c10, 0x0018, 0x0000}, 2, "JSR 0x0018"},
		{[]uint16{0x7dc1, 0x001a, 0x0000}, 2, "SET PC, 0x001a"},
		{[]uint16{0x9037, 0x0000, 0x0000}, 1, "SHL X, 0x04"},
		{[]uint16{0x61c1, 0x0000, 0x0000}, 1, "SET PC, POP"},
		{[]uint16{0x7dc1, 0x001a, 0x0000}, 2, "SET PC, 0x001a"},
	}
